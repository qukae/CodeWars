...
PaginationHelper.prototype.pageItemCount = function(pageIndex) {
  if (pageIndex >= this.pageCount() || pageIndex < 0) {
    ...
    const pageContent = new Array(this.pageCount())
    let items = this.itemCount()
    for (let i = 0; i < pageContent.length; i++) {
      if (items >= this.itemsPerPage) {
      pageContent[i] = this.itemsPerPage;
...
PaginationHelper.prototype.pageIndex = function(itemIndex) {
  ...
  return (Math.ceil(this.collection.slice(0, itemIndex).length/this.itemsPerPage)) -1;
}
